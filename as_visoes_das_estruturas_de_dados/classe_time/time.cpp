...
#include <iostream>  // Para usar a função std::cout. (perceba que quando é um arquivo do proprio cpp, usamos o <>)
#include "time.h"    // Para visualizar a classe Time. (ja quando criamos, usamos "")
...
/* 
   Getters
 */
int Time::getHour() const { // quando metodos não atrapalah atributos, usamos const
  return hour;
}
...
/*
  Setters -> setamos os atributos da classe.
 */
...
void Time::print() const { // Método para imprimir o horário no formato HH:MM:SS
  cout << hour << ":" << minute << ":" << second << endl;     
}

void Time::nextSecond() { // Método para incrementar o horário em um segundo
  second += 1;
  if (second >= 60) {
    second = 0;
    minute += 1;
